...
if [[ -n "$RAILWAY_ENVIRONMENT" ]]; then
    echo "Detected Railway environment - using apt-get without sudo"
    
    # Update package manager
    apt-get update
    
    # Install texlive (Railway containers have root access)
    apt-get install -y texlive-full texlive-fonts-extra texlive-latex-extra
    
    echo "LaTeX installation complete in Railway!"
...
# Install/Update required packages (skip in Railway as texlive-full includes them)
...
if pdflatex test.tex; then
    ...
    # Don't exit 1 in Railway to allow deployment to continue
    if [[ -z "$RAILWAY_ENVIRONMENT" ]]; then
        exit 1
    fi
fi
